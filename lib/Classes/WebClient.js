...
class WebClient extends EventEmitter {
    ...
    constructor(client, options) {
        ...
        this.Headers['User-Agent'] = `${this.Client.Product.name}/${this.Client.Product.version} (e621:bjorkin)`
     ...
     * @property {string} User-Agent Default Value `esix-api/0.x.x (e621:bjorkin)`
     * @description Headers that get merged with the default node:https request headers.
     */
    Headers = {
        "User-Agent": `esix-api/0.x.x (e621:bjorkin)`
    }
    ...
    request(options, resolve, reject) {
        if (options.path.toString().includes("?")) {
            options.path += "&" + this.Auth.toString();
        } else {
            options.path += "?" + this.Auth.toString();
        }
