...
class Client extends EventEmitter {
    ...
    async Favorite(post_id, status) {
        ...
        if (this.PostCache[post_id] == undefined) {
            await this.GetPost(post_id)
        }
        this.PostCache[post_id].emit('update')
        return this.PostCache[post_id]
    }
    ...
    async Vote(post_id, int) {
        ...
        if (this.PostCache[post_id] != undefined)
            await this.GetPost(post_id)
        this.PostCache[post_id].data.score = json
        return json
    }
    ...
    async GetPost(post_id) {
        if (this.PostCache[post_id] == undefined) {
            ...
            if (this.PostCache[post_id] == undefined)
                this.PostCache[post_id] = new PostObject(this, post_id, json.post, null)
        } else {
            await this.PostCache[post_id].emit('update')
        }
        return this.PostCache[post_id]
    }
